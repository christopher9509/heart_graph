...
train = pd.read_csv('/nas1/yongk/BK/train_SMOTE.csv') #sftp://hjae@165.132.195.253/home/hjae/heart/dataset/train_SMOTE_not.csv
train = train.iloc[:-1,:]
...
def split_train(n, df0):
    for i in range(len(df0)//n):
       ...
       #temp = pd.concat([temp_x, temp_y], axis = 1)
        temp_shuffled = sklearn.utils.shuffle(temp_x, random_state=0)
        
        globals()['train_temp_{}'.format(i)] = temp_shuffled
...
np.savez_compressed('/nas1/yongk/BK/dataset/top_1p_smt_s_train_feats_4096.csr'.format(i), sparse_X)

with open('/nas1/yongk/BK/dataset/top_1p_smt_s_train_feats_4096.csr'.format(i), 'wb') as f:
    pickle.dump(sparse_X, f, pickle.HIGHEST_PROTOCOL)

#내보내기 - (2) : node-labels
np.save('/nas1/yongk/BK/dataset/top_1p_smt_s_train_labels_4096.npy'.format(i), node_labels)

with open('/nas1/yongk/BK/dataset/top_1p_smt_s_train_labels_4096.npy'.format(i), 'wb') as f:
...
with open("/nas1/yongk/BK/dataset/top_1p_smt_s_train_graph_4096.json", "w") as f:
    json.dump(composed_graph, f)

#내보내기 - (4) : graph_ids
np.save('/nas1/yongk/BK/dataset/top_1p_smt_s_train_graph_id_4096.npy'.format(i), node_ids)

with open('/nas1/yongk/BK/dataset/top_1p_smt_s_train_graph_id_4096.npy'.format(i), 'wb') as f:
