...
class MyEnsemble(nn.Module):
    ...
    def forward(self, demo_graph, habit_graph, under_graph):
        ...
        head1 = self.gat0((demo_graph)).x
        head2 = self.gat1((habit_graph)).x
        head3 = self.gat2((under_graph)).x
...
class Net(torch.nn.Module):
    def __init__(self, num_of_layers, nfeat, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
        ...
        self.sc2 = GatedSkipConnection(nfeat, num_features_per_layer[2] * num_heads_per_layer[2])
        ...
        self.classifier = nn.Linear(num_features_per_layer[3], 1)
    ...
    def forward(self, x):
        ...
        x = Data(x=resi_1, edge_index=x.edge_index)
        
        x = self.layer1(x)
        resi_2 = self.sc2(residual, x.x)
        batch_2 = self.batch2(resi_2)
        resi_2 = self.dropout(batch_2)
        x = Data(x=resi_2, edge_index=x.edge_index)

        x = self.layer2(x)
        x.x = self.dropout(x.x)
        x = Data(x=x.x, edge_index=x.edge_index)
