...
class Net(torch.nn.Module):
    def __init__(self, num_of_layers, nfeat, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
        ...
        if GAT_layer == GATLayer :
            ...
            gat_layers.append(layer0)
            gat_layers.append(layer1)
            gat_layers.append(layer2)
        
            self.gnn_net = nn.Sequential(*gat_layers)
            ...
            gatv2_layers.append(layer0)
            gatv2_layers.append(layer1)
            gatv2_layers.append(layer2)
        
            self.gnn_net = nn.Sequential(*gatv2_layers)
...
class GATv2Layer(GAT):
    ...
    def forward(self, data):
        ...
        nodes_features_proj_l_repeat = nodes_features_proj_l.repeat(num_of_nodes, 1, 1)
        nodes_features_proj_r_interleave = nodes_features_proj_l.repeat_interleave(num_of_nodes, 1)
        
        nodes_features_proj_sum = nodes_features_proj_l_repeat + nodes_features_proj_r_interleave
        
        nodes_features_proj_sum = nodes_features_proj_sum.view(num_of_nodes, self.num_of_heads, self.num_out_features)
        ...
        # shape = (N, NH, FOUT) * (1, NH, FOUT) -> (N, NH, 1) -> (N, NH) because sum squeezes the last dimension
        # Optimization note: torch.sum() is as performant as .sum() in my experiments
        scores_source = (nodes_features_proj_sum * self.scoring_fn_source).sum(dim=-1) # g_i / torch.Size([4096, 4])
        scores_target = (nodes_features_proj_sum * self.scoring_fn_target).sum(dim=-1) # g_j / torch.Size([4096, 4])
