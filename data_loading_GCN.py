...
def load_graph_data(training_config, device):
    ...
    if dataset_name == DatasetType.CORA.name.lower():  # Cora citation network
        ...
        return node_features, node_labels, topology, train_indices, val_indices, test_indices, adj

    elif dataset_name == DatasetType.LENDING.name.lower():  # Protein-Protein Interaction dataset

        # Instead of checking PPI in, I'd rather download it on-the-fly the first time it's needed (lazy execution ^^)
        if not os.path.exists(LENDING_PATH):  # download the first time this is ran
            os.makedirs(LENDING_PATH)

            # Step 1: Download the ppi.zip (contains the PPI dataset)
            zip_tmp_path = os.path.join(LENDING_PATH, 'ppi.zip')
            download_url_to_file(LENDING_URL, zip_tmp_path)

            # Step 2: Unzip it
            with zipfile.ZipFile(zip_tmp_path) as zf:
                zf.extractall(path=LENDING_PATH)
            print(f'Unzipping to: {LENDING_PATH} finished.')
        ...
        for split in splits:
            ...
            node_features_1 = np.load(os.path.join(LENDING_PATH, f'top_1p_smt_s_{split}_feats.csr'), allow_pickle=True)
            ...
            node_labels1 = np.load(os.path.join(LENDING_PATH, f'top_1p_smt_s_{split}_labels.npy'), allow_pickle=True)
            node_labels2 = torch.from_numpy(np.array(node_labels1).astype(np.int8))
            node_labels = torch.unsqueeze(node_labels2, 1)

            # PPI has 121 labels and each node can have multiple labels associated (gene ontology stuff)
            # SHAPE = (NS, 121)
            #node_labels = np.load(os.path.join(LENDING_PATH, f'{split}_labels.npy'))

            # Graph topology stored in a special nodes-links NetworkX format
            nodes_links_dict = json_read(os.path.join(LENDING_PATH, f'top_1p_smt_s_{split}_graph.json'))
            #print("nodes_links_dict shape: ",nodes_links_dict.shape)
            
            #adj = build_adj(adjacency_list_dict)
            ...
            graph_ids = np.load(os.path.join(LENDING_PATH, F'top_1p_smt_s_{split}_graph_id.npy'), allow_pickle=True)
            ...
            for graph_id in range(np.min(graph_ids), np.max(graph_ids) + 1):
                ...
                graph = collection_of_graphs.subgraph(graph_node_ids)#graph_node_ids  # returns the induced subgraph over these nodes
...
#                temp_adj = build_adj_from_graph(graph)
#                 print("="*40)
#                 print(temp_adj.shape)
                
#                 adj_list.append(torch.tensor(temp_adj, dtype=torch.float))
...
#                 print(len(graph_list))
        ...
        if training_config['lending_load_test_only']:
            data_loader_test = GraphDataLoader(
                ...
                graph_list[num_graphs_per_split_cumulative[0]:num_graphs_per_split_cumulative[1]],
            ...
            data_loader_val = GraphDataLoader(
                ...
                graph_list[num_graphs_per_split_cumulative[1]:num_graphs_per_split_cumulative[2]],
                batch_size=training_config['batch_size'],
            ...
            data_loader_test = GraphDataLoader(
                ...
                graph_list[num_graphs_per_split_cumulative[2]:num_graphs_per_split_cumulative[3]],
                batch_size=training_config['batch_size'],
                shuffle=False
            )
...
class GraphDataLoader(DataLoader):
    ...
    """
    def __init__(self, node_features_list, node_labels_list, graph_list, batch_size=1, shuffle=False): #edge_index_list,

        graph_dataset = GraphDataset(node_features_list, node_labels_list, graph_list) # edge_index_list
        
#         print("="*40)
#         print(len(graph_dataset))
#         print("len adj list: ", len(adj_list))
...
class GraphDataset(Dataset):
    ...
    """
    def __init__(self, node_features_list, node_labels_list, graph_list): #edge_index_list,
...
#         self.adj_list = adj_list
    ...
    def __getitem__(self, idx):  # we just fetch a single graph
        return self.node_features_list[idx], self.node_labels_list[idx], self.graph_list[idx]  #self.edge_index_list[idx]
...
#                 temp_adj = build_adj_from_graph(graph)
    for features_labels_edge_index_tuple in batch:
        ...
        #print(features_labels_edge_index_tuple)
...
#         edge_index_list.append(edge_index + num_nodes_seen)  # very important! translate the range of this component
        ...
        graph = features_labels_edge_index_tuple[2]
        graph_list.append(graph)
        #total_graph = nx.disjoint_union(start_graph, temp_graph)
        #adj_list.append(features_labels_edge_index_tuple[3])
...
#     edge_index = torch.cat(edge_index_list, 1)
    total_graph = nx.disjoint_union_all(graph_list)
    
    #temp_graphs = nx.from_edgelist(edge_index)
    adj = build_adj_from_graph(total_graph)
    #adj = torch.stack(adj_list)
    
    return node_features, node_labels, adj #edge_index
...
#with no use at this time 
def build_adj(nodes_links_dict):
    nx_graph = nx.from_dict_of_lists(nodes_links_dict)
    adj = nx.adjacency_matrix(nx_graph)
    adj = adj.tocoo()  # convert to COO (COOrdinate sparse format)
...
def build_adj_from_graph(graph):
    ...
    #adj = nx.adjacency_matrix(nx_graph)
    adj = adj.tocoo().astype(np.float32)  # convert to COO (COOrdinate sparse format)
