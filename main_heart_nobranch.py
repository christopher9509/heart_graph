...
from models.GAT_Custom_copy import Net

from data_loading_copy import load_graph_data
#from utils.data_loading import load_graph_data_2
from utils.constants import *
from utils.utils import FocalLossV2
...
def get_main_loop(config, gat, sigmoid_cross_entropy_loss, optimizer, scheduler, patience_period, time_start):
...
#             edge_indexs = torch.squeeze(edge_index)            
            ...
            if phase == LoopPhase.TRAIN:
                ...
                optimizer.step()  # apply the gradients to weights             # scaled update      
                scheduler.step()  
            ...
            #macro_f1 = f1_score(pred, gt, average = 'macro', num_classes=1)
...
#     print("="*30)
    ...
    #pos_weight = (y==1.).sum()/y.sum()
    #pos_weight=torch.tensor([0.9140],device=device)
...
def get_training_args():
    ...
    parser.add_argument("--gpu", type=int, help="number of gpu to use", default=0)
    parser.add_argument("--num_of_epochs", type=int, help="number of training epochs", default=15)
    parser.add_argument("--patience_period", type=int, help="number of epochs with no improvement on val before terminating", default=200000)
    ...
    gat_config = {
        ...
        "num_features_per_layer": [34, 16, 16, 16],  #(34) 64 would also give ~0.975 uF1!
        ...
        "layer_type": LayerType.IMP2  # the only implementation that supports the inductive setting
    }
